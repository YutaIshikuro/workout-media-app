...
- **1 piece = tasks.md の親タスク単位**（サブタスクの束）。1 タスク単位は worktree と context の立ち上げコストが実装量を上回り、1 スペック単位は差分が大きすぎてレビューが形骸化する
- **review → fix が 3 往復したら人間にエスカレーションする。** TAKT のループ検出は保険として重ねる
- **tasks.md の各タスクは単体で読めば実装できる状態にする。** Codex は会話文脈も steering も自動では読まない。受け入れ基準、参照すべきファイルパス、対応する要件 ID をタスク本文に含める
