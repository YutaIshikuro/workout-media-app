...
- [ ] 1.4 リポジトリ衛生を設定する
  ...
  - 依存ツリーを一覧し、アナリティクス・クラッシュレポート・テレメトリに類するパッケージが含まれないことを確認する
  - 観測可能な完了: `git status` に生成物と秘密情報の候補が現れず、依存ツリーの確認結果が出力として残る
