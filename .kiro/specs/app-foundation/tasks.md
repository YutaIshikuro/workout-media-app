...
- [ ] 5.4 iOS Simulator の E2E ジョブを構成する
  ...
  - **`ffmpeg-static` を devDependency から外す。** 5.2 が追加したが、`scripts/make-seed-media.sh` は `npx` 経由で解決しており依存を必要としない。要求しているのは `tests/seed-media-script.test.ts` だけであり、そのために CI の `npm ci` が毎回 77MB のバイナリを外部ホストから取得している。判定は依存なしで行える（MP4 コンテナの `avc1` / `mp4a` アトムの有無を読む）ので、テストをその形に書き換えて依存を削除する
